...
function getShopConfig()
{
	...
	$sql = "SELECT sc_name, sc_address, sc_phone, sc_email, sc_shipping_cost, sc_order_email, cy_symbol 
			FROM tbl_shop_config sc, tbl_currency 
...
function thai_currency($amount) {
		if ($amount < 0) {
			$print_number = str_replace('-', '<span style="color:red;">- ', number_format($amount,2,".", ",")) . "</span>";
		} else {
			$print_number = number_format($amount, 2,".",",");
		}
		return $print_number;
}
...
function getPagingLink($sql, $itemPerPage = 10, $strGet = '')
{
	...
	if ($totalPages > 1) {
		...
		if ($pageNumber > 1) {
			$page = $pageNumber - 1;
			if ($page > 1) {
				$prev = " <a href=\"$self?page=$page&$strGet/\">[ก่อนหน้า]</a> ";
			} else {
				$prev = " <a href=\"$self?$strGet\">[ก่อนหน้า]</a> ";
			}	
				
			$first = " <a href=\"$self?$strGet\">[เพจแรก]</a> ";
		} else {
