...
$sql = "SELECT pd.cat_id, pd_name, pd_description, pd_price, pd_wholesale, pd_pv, pd_qty, pd_image, pd_thumbnail
...
//สร้าง list ประเภทสินค้า โดยเรียกใช้ฟังก์ชัน buildCategoryOptions() พร้อมส่งประเภทในปัจจุบันไปด้วย
$categoryList = buildCategoryOptions($cat_id);
/*
//ดึงรายชื่อประเภทสินค้าจากฐานข้อมูล
$sql= "SELECT cat_id, cat_parent_id, cat_name FROM tbl_category ORDER BY  cat_id";
$result= dbQuery($sql);

//เก็บรายชื่่อประเภทสินค้าลงอาเรย์
$categories = array();
...
//แสดงประเภทสินค้าในแบบ Dropdown list
$categoryList = '';
foreach($categories as $key=>$value){
    $name = $value['name'];
    $children = $value['children'];
    $categoryList.="<optgroup lable=\"$name\">";
       foreach($children as $child){
      $categoryList.="<option value={$child['id']} ";
        if($child['id']==$cat_id){
          $categoryList.="selected";

        }
        $categoryList.=">{$child['name']}</option>";
    }
...
?>	 
  ...
  <tr> 
   <td width="150" class="label">Wholesale</td>
   <td class="content"><input name="txtWholesale" type="text" class="box" id="txtWholesale" value="<?php echo $pd_wholesale; ?>" size="10" maxlength="7" ></td>
  <tr> 
   <td width="150" class="label">Pv</td>
   <td class="content"><input name="txtPv" type="text" class="box" id="txtPv" value="<?php echo $pd_wholesale; ?>" size="10" maxlength="7"> </td>
  </tr>
  <tr> 
